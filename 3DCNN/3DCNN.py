...
xtrain, xtest, ytrainH, ytestH=train_test_split(image_data, labels, test_size=0.15)
...
ytestP = np.concatenate([([i]*2) for i in [2, 5, 9]], axis=0) #(30,) Werte zwischen 2 und 9
ytrainP = np.concatenate([([i]*10) for i in [2, 5, 9]], axis=0) #(30,) Werte zwischen 2 und 9
...
model.fit(
	...
	y=[ytrainH, ytrainP],
	validation_data=([testA, testB, testC, testD, testE], [ytestH, ytestP]),
...
x_ax = range(len(ypred[0]))
plt.scatter(x_ax, ytestH, s=5, color="blue", label="original")
plt.plot(x_ax, ypred[0], lw=0.8, color="red", label="predicted")
plt.legend()
plt.title("Wave Height")
plt.show()

x_ax = range(len(ypred[1]))
plt.scatter(x_ax, ytestP, s=5, color="blue", label="original")
plt.plot(x_ax, ypred[1], lw=0.8, color="red", label="predicted")
plt.legend()
plt.title("Period")
plt.show()
